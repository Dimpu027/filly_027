...
```
REACT_ENABLED_APPS=ohcnetwork/care_payroll_fe@localhost:4173/assets/remoteEntry.js
```
...
| `public/locale/en.json` | i18n keys, all prefixed `payroll__`. |

## Conventions

- Every entry in `components` and `routes` must be `lazy()` — the manifest chunk loads on
  every page of `care_fe`.
- Every user-facing string is `t("payroll__key")`, defined in this repo's `en.json`.
